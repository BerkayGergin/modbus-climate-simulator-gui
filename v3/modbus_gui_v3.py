...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.log("INFO", "SYSTEM", "ClimaLab HMI initialized.")
        self.log("INFO", "SYSTEM", "Ready for Modbus TCP connection.")
    ...
    def build_diag_box(self, title, initial):
        ...
        l.addWidget(val); setattr(self, f"diag_{title.lower()}_value", val)
        return box
    ...
    def set_connection_state(self, connected: bool):
        if connected:
            ...
            if hasattr(self, "diag_connection_value"):
                self.diag_connection_value.setText("ONLINE")
        else:
            ...
            if hasattr(self, "diag_connection_value"):
                self.diag_connection_value.setText("OFFLINE")
    ...
    def poll_data(self):
        ...
        t0 = perf_counter()
        try:
            # Read each Modbus area independently so one transient failure does
            # not immediately tear down a healthy TCP session.
            ir = self.client.read_input_registers(address=0, count=12, device_id=self.settings.device_id)
            if ir.isError():
                raise RuntimeError(f"FC04 INPUT REGISTERS failed: {ir}")

            hr = self.client.read_holding_registers(address=0, count=6, device_id=self.settings.device_id)
            if hr.isError():
                raise RuntimeError(f"FC03 HOLDING REGISTERS failed: {hr}")

            coils = self.client.read_coils(address=0, count=3, device_id=self.settings.device_id)
            if coils.isError():
                raise RuntimeError(f"FC01 COILS failed: {coils}")

            raw_ir = list(ir.registers)
            raw_hr = list(hr.registers)
            raw_coils = list(coils.bits)
            if len(raw_ir) < 12 or len(raw_hr) < 6 or len(raw_coils) < 3:
                raise RuntimeError(
                    f"Incomplete Modbus response: IR={len(raw_ir)}/12 HR={len(raw_hr)}/6 COILS={len(raw_coils)}/3"
                )

            self.response_ms = (perf_counter() - t0) * 1000.0
            self.poll_failures = 0
            s = self.decode_snapshot(raw_ir, raw_hr, raw_coils)
            ...
            self.diag_latency_value.setText(f"{self.response_ms:.1f} ms")
            self.diag_polling_value.setText(f"{self.settings.poll_ms} ms")
            if hasattr(self, "diag_connection_value"):
                self.diag_connection_value.setText("ONLINE")

        except Exception as exc:
            self.poll_failures += 1
            self.log("ERROR", "POLL", f"Attempt {self.poll_failures}/{self.max_poll_failures}: {exc}")
            if hasattr(self, "diag_connection_value"):
                self.diag_connection_value.setText(f"RETRY {self.poll_failures}")
            # Keep the TCP session alive for a few transient failures. Only
            # disconnect after repeated failures; this also makes write/control
            # actions usable while a single read is hiccupping.
            if self.poll_failures >= self.max_poll_failures:
                self.log("ERROR", "CONNECT", "Repeated Modbus polling failures; disconnecting session.")
                self.disconnect_modbus()
        finally:
            self.polling = False
    ...
    def decode_snapshot(ir: list[int], hr: list[int], coils: list[bool]):
        return {
            ...
            "fan": hr[1],
            "mode": hr[2],
            "command_revision": hr[3] if len(hr) > 3 else 0,
            "applied_revision": hr[4] if len(hr) > 4 else 0,
            "command_status": hr[5] if len(hr) > 5 else 0,
    ...
    def apply_snapshot(self, s: dict):
        ...
        self.diag_runtime_value.setText(f"{s['runtime']:.0f} h")
        ...
        # Real device-level command acknowledgement. The server mirrors the
        # command revision into Applied Revision after consuming the command.
        if self.pending_command_revision is not None:
            if s.get("applied_revision") == self.pending_command_revision and s.get("command_status") == 1:
                if not self.command_confirmed_logged:
                    self.control_feedback.setText(
                        f"✓ Device confirmation received. Command #{self.pending_command_revision} applied successfully."
                    )
                    self.log("SUCCESS", "CONTROL", f"Device confirmed command #{self.pending_command_revision}.")
                    self.command_confirmed_logged = True
                    self.pending_command_revision = None
            elif self.command_sent_at is not None:
                elapsed = (datetime.now() - self.command_sent_at).total_seconds()
                if elapsed > 5.0:
                    self.control_feedback.setText(
                        f"⚠ Device confirmation timeout for command #{self.pending_command_revision}."
                    )
                    self.log("WARNING", "CONTROL", f"Confirmation timeout for command #{self.pending_command_revision}.")
                    self.pending_command_revision = None
    ...
    def apply_control_commands(self):
        ...
        try:
            ...
            self.command_revision_counter = (self.command_revision_counter % 65534) + 1
            revision = self.command_revision_counter
            ...
            # HR3 is a command revision; the simulator echoes it into HR4 once
            # the device loop has consumed the new configuration.
            self.safe_write_register(3, revision)

            self.pending_command_revision = revision
            self.command_sent_at = datetime.now()
            self.command_confirmed_logged = False
            self.control_feedback.setText(
                f"✓ Commands sent. Waiting for device confirmation of command #{revision}…"
            )
            self.log("WRITE", "CONTROL", f"CMD#{revision} SET={setpoint:.1f}°C FAN={fan}% MODE={MODE_NAMES[mode]} ENABLE={enable}")
